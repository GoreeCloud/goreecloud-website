...
Current website package: **v5.7 — production-readiness hardening in progress**
...
Repository-only material such as CI validators, GitHub metadata, contributor documentation, development artifacts, and unapproved local files is kept separate from the generated public deployment artifact.
...
- `scripts/validate_repository_hygiene.py` — rejects sensitive file types, private-key material, selected reusable credential signatures, symlinks, editor artifacts, and missing local-ignore protections
...
- `scripts/validate_workflow_security.py` — enforces immutable GitHub Actions references, least privilege, and retention of required production gates
...
The generated artifact must contain only the intentional public surface. Files such as `README.md`, `SECURITY.md`, `.github/`, `scripts/`, `.gitignore`, local environment files, private keys, development files, and unapproved artwork are repository or local concerns and must never be copied into `dist/`.
...
The repository-hygiene validator provides an additional prevention layer. It rejects tracked/source symlinks; environment and secret-bearing configuration files; common private-key and certificate-container file types; SSH private-key filenames; editor/merge artifacts; selected operating-system metadata; private-key blocks; and several high-confidence reusable token/credential signatures. This is a targeted prevention control, not a substitute for careful review or dedicated history scanning. The source-license/publication work in issue #5 still includes repository-history and publication-boundary review before any visibility change.
...
The workflow self-validator requires the repository-hygiene, Glaze UI, accessibility, privacy, security, browser-origin, public-surface, artifact, performance, and remote-verifier configuration gates to remain wired into CI. Removing one of those checks therefore fails the workflow-supply-chain validation rather than silently weakening production readiness.
