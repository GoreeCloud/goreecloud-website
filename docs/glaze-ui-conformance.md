...
At Wide widths, the content canvas grows to a 1480 CSS-pixel maximum with 40-pixel semantic gutters. The hero typography and visual surface scale up without becoming full-width; service, development, and social collections use four balanced columns; and platform cards retain readable four-column density. The eight-account social collection flows as two complete four-card rows without a legacy GitHub-only column override, and a final two-card development row is centered across the Wide grid rather than leaving the right half visually empty.
...
### Rendered desktop preflight and review evidence
...
For both 1280 × 900 and 1600 × 1000, the preflight renders System, Light, and Dark appearance modes and checks the semantic desktop gutter/container relationship, hero minimum geometry, horizontal overflow, primary-navigation containment, image loading, and Wide four-column collection density. It also exercises reduced-motion and increased-contrast media fallbacks and requires each representative browser render to produce a valid nontrivial PNG screenshot payload.

`scripts/capture_desktop_evidence.py` then performs a separate read-only evidence pass after the deployed-render gate. It preserves six representative viewport screenshots plus Wide full-page Light and Dark screenshots in a GitHub Actions artifact retained for 14 days. Appearance changes are allowed 0.4 seconds to settle before evidence is captured so reviewed images show the steady Glaze UI state rather than an intermediate transition frame. The evidence manifest records screenshot dimensions, SHA-256 digests, representative layout metrics, and the capture-settle interval. These files remain outside the isolated Cloudflare `dist/` artifact and are not public website assets or production runtime dependencies.
