...
const CURRENT_PUBLIC_PROJECTS = [
  {
    name: 'GoreeVault Server',
    description: 'Maintained Vaultwarden-derived credential server advancing toward a GoreeCloud-owned multi-user security product with Glaze UI and release-readiness gates.',
    repository: 'https://github.com/GoreeCloud/goreevault-server',
  },
  {
    name: 'GoreeCloud Feed',
    description: 'Native RSS client layer over FreshRSS with a shared Glaze UI experience for web, Linux desktop, and Android while FreshRSS remains the authoritative backend.',
    repository: 'https://github.com/GoreeCloud/goreecloud-rss',
  },
  {
    name: 'GoreeCloud Gallery',
    description: 'Maintained Android gallery application focused on offline-first local media browsing, privacy, reproducible builds, and Glaze UI release readiness.',
    repository: 'https://github.com/GoreeCloud/goreecloud-gallery',
  },
];

function createProjectCard({ name, description, repository }) {
  const card = document.createElement('div');
  card.dataset.project = name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');
  ...
  link.textContent = 'View public repository →';

  card.append(title, summary, link);
  return card;
}

function reconcilePublicProjectPortfolio() {
  const projectGrid = document.querySelector('#development .development-grid');
  if (!projectGrid) return;

  const bookmarkCard = Array.from(projectGrid.children).find(
    (card) => card.querySelector('strong')?.textContent?.trim() === 'GoreeCloud Bookmarks',
  );

  if (bookmarkCard) {
    const summary = bookmarkCard.querySelector('span');
    const status = bookmarkCard.querySelector('small');
    if (summary) {
      summary.textContent = 'Maintained Linkwarden-based bookmark, reading, annotation, and web-preservation platform with a companion browser extension.';
    }
    status?.remove();

    if (!bookmarkCard.querySelector('a')) {
      const link = document.createElement('a');
      link.href = 'https://github.com/GoreeCloud/goreecloud-bookmarks';
      link.target = '_blank';
      link.rel = 'noopener noreferrer';
      link.textContent = 'View public repository →';
      bookmarkCard.append(link);
    }
  }

  const existingNames = new Set(
    Array.from(projectGrid.querySelectorAll('strong')).map((title) => title.textContent?.trim()),
  );

  for (const project of CURRENT_PUBLIC_PROJECTS) {
