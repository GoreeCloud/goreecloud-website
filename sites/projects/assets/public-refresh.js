...
function ensureAugust25Entries(){
  if(!entries.some(entry=>entry.name==='GoreeCloud AI'))entries.push({name:'GoreeCloud AI',repo:'goreecloud-ai',category:'Platform & Administration',model:'Native',kind:'Application',status:'Repository established · initial source foundation',role:'The first-party repository is established for a native Ollama front end designed with Glaze UI and secured through Wardveil Security. The broader Workspace, knowledge, RAG, research, file, tool, and orchestration scope remains active development rather than a Stable or production claim.',visibility:'Public'});
  if(!entries.some(entry=>entry.name==='GoreeCloud Mesh'))entries.push({name:'GoreeCloud Mesh',repo:'goreecloud-mesh',category:'Shared Foundations',model:'Coordination and governance plane',kind:'Foundation',status:'Active development · scoped API + durable Everkeep evidence',role:'Mesh now includes fail-closed scoped authorization for private mutating APIs, producer-bound evidence-validity contracts, and durable Everkeep recovery-evidence persistence while preserving the authority boundaries of Identity, Wardveil Security, Privacy Shield, Everkeep, Glaze UI, and specialized applications.',visibility:'Public'});
}
