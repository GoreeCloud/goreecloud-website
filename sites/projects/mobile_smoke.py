...
def inspect_mobile(session_id: str, browser: str, width: int) -> None:
    ...
    require(float(state.get("filterMinHeight",0))>=47.5,f"Projects mobile filter targets are below the Glaze UI 2.2 48px floor in {browser} at {width}px: {state}")
    require(float(state.get("navMinHeight",0))>=47.5,f"Projects mobile navigation targets are below the Glaze UI 2.2 48px floor in {browser} at {width}px: {state}")
    require(float(state.get("themeMinHeight",0))>=47.5,f"Projects mobile appearance targets are below the Glaze UI 2.2 48px floor in {browser} at {width}px: {state}")
    ...
    require(str(state.get("cardBackdrop","none")) in ("none",""),f"Projects durable mobile cards must remain solid under Glaze UI 2.2 in {browser} at {width}px: {state}")
...
def run(target: str,browser: str) -> int:
    ...
    try:
        ...
        print(f"Projects mobile {browser} branding/layout/Glaze UI 2.2 smoke passed for {target}: {target_url}")
        return 0
