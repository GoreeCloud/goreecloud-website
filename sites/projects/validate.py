...
required = [
    ...
    "assets/styles.css", "assets/mobile-refresh.css", "assets/glaze-ui-2.2.0.css",
...
glaze = (SITE / "assets/glaze-ui-2.2.0.css").read_text(encoding="utf-8")
headers = (SITE / "_headers").read_text(encoding="utf-8")
combined = html + js + icons + readme
active_direction = html + js
...
for needle in [
    "Suite applications", "Shared foundations", "Glaze UI 2.2", "Privacy Shield",
    "Wardveil Security", "Everkeep", "GoreeCloud Mesh", "GoreeCloud Identity",
    "GoreeCloud Manager", "seven Integral Platform Systems",
...
for marker in (
    ...
    if marker not in glaze:
        raise SystemExit(f"Projects Glaze UI 2.2 web-layer marker missing: {marker}")
for forbidden in ("raw.githubusercontent.com", "@import url(\"http", "@import url('http"):
    if forbidden in glaze:
        raise SystemExit(f"Projects Glaze UI must remain same-origin/local: {forbidden}")
...
for stylesheet in ["/assets/mobile-refresh.css?v=20260827-mobile2", "/assets/glaze-ui-2.2.0.css"]:
...
print("GoreeCloud Projects source-native current portfolio, Glaze UI 2.2, responsive hardening, and unified branding validation passed")
