...
for stale in [
    ...
    if stale in combined: raise SystemExit(f"superseded Projects direction remains public: {stale}")
...
for needle in [branding_repo,"catalog.json","synchronized publication derivatives","Sentinel Fold","systems/goreecloud-mesh/goreecloud-mesh-mark.svg","Weave"]:
...
for source in [
    "products/manager/app-icon.svg",
    "products/browser/app-icon.svg",
    "products/ai/app-icon.svg",
    "systems/glaze-ui/glaze-ui-mark.svg",
    "systems/everkeep/everkeep.svg",
...
mesh_mapping="'goreecloud-mesh':['systems/goreecloud-mesh/goreecloud-mesh-mark.svg','/assets/goreecloud-mesh-mark.svg','approved']"
if mesh_mapping not in icons:
    raise SystemExit("Projects Mesh must use the approved unified-catalog Weave artwork")
...
if "Security Center · Sentinel Fold" not in html or "Sentinel Fold primary mark" not in html:
    raise SystemExit("Projects Wardveil foundation identity must identify Sentinel Fold as the approved primary mark")
...
for script in ["/assets/app.js?v=20260827-cache2","/assets/public-refresh.js?v=20260827-cache2","/assets/icon-refresh.js?v=20260828-identities1"]:
